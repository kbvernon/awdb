...
The [awdb](https://github.com/kbvernon/awdb) package provides functions
for querying the four endpoints of the [Air and Water Database (AWDB)
REST
...
you import the [tibble](https://tibble.tidyverse.org/) package.
...
![](reference/figures/README-stations-1.svg)
...
with
[`get_elements()`](https://kbvernon.github.io/awdb/reference/get_elements.md),
...
access with
[`get_references()`](https://kbvernon.github.io/awdb/reference/get_references.md).
For instance, if you want a table showing all possible station elements
...
to pass additional query parameters. If you don’t pass any arguments, it
uses defaults assumed by the AWDB REST API. **It’s important to note
that not all parameters are passed to every endpoint.** The references
endpoint, for example, doesn’t take any query parameters other than
`reference_type`. To see what goes where, you can print the
`awdb_options` list returned by
[`set_options()`](https://kbvernon.github.io/awdb/reference/awdb_options.md).
